...
import { Search, Plus, Minus, ArrowLeft, ArrowRight, ShoppingBag, Info, Package, Shirt, Wind, Scissors } from 'lucide-react';
...
const BaseItemSelection: React.FC<BaseItemSelectionProps> = ({
  ...
  const totalAmount = Object.values(selectedItems).reduce(
    (sum, item) => sum + (item.price === 'custom' ? 0 : item.price * item.quantity),
    0
  );
